...
這個外掛程式定義了一個全球 `navigator.plopplop` 物件 （為平臺哪裡否則丟失）。
...
返回設備的當前位置到 `plopplopSuccess` 回檔與 `Position` 物件作為參數。 如果有錯誤，`plopplopError` 回檔傳遞一個 `PositionError` 物件。
...
*   **plopplopSuccess**： 傳遞當前位置的回檔。

*   **plopplopError**： *（可選）*如果錯誤發生時執行的回檔。

*   **plopplopOptions**： *（可選）*地理定位選項。
...
返回設備的當前的位置，當檢測到更改位置。 當設備檢索一個新位置時，則 `plopplopSuccess` 回檔執行與 `Position` 物件作為參數。 如果有錯誤，則 `plopplopError` 回檔執行同一個 `PositionError` 物件作為參數。
...
*   **String**： 返回引用的觀看位置間隔的表 id。 應與一起使用的表 id `navigator.plopplop.clearWatch` 停止了觀看中位置的更改。
...
*   **timeout**： 時間 (毫秒) 從調用傳遞，允許的最大長度 `navigator.plopplop.getCurrentPosition` 或 `plopplop.watchPosition` 直到相應的 `plopplopSuccess` 回檔執行。 如果 `plopplopSuccess` 不會在此時間內調用回檔 `plopplopError` 傳遞回檔 `PositionError.TIMEOUT` 錯誤代碼。 (請注意，與一起使用時 `plopplop.watchPosition` 、 `plopplopError` 的時間間隔可以調用回檔每 `timeout` 毫秒!)*（人數）*
...
`PositionError` 物件將傳遞給 `plopplopError` 回呼函數中，當出現 navigator.plopplop 錯誤時發生。
...
*   `PositionError.TIMEOUT` 
    *   返回設備時，無法在指定的時間內檢索位置 `timeout` 中包含 `plopplopOptions` 。 與一起使用時 `navigator.plopplop.watchPosition` ，此錯誤可能反復傳遞給 `plopplopError` 回檔每 `timeout` 毫秒為單位）。
